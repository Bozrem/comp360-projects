...
;;; Answer: no
;;; Why: the recursive is not the last evaluation
...
;;; Answer: yes
;;; Why: the recursive call is the last evaluation
...
;;; Answer: no
;;; Why: even though it's tail recursive for even, it isn't for else
...
; Non-tr has the stack growing at each recursive call, tr version doesn't (constant memory usage)
;;; 2. Why does the tail-recursive version stay at depth 1?
; Racket has TCO, which means the stack frame gets reused during tail recursion
;;; 3. What would happen with a list of 10,000 elements in each version?
; Memory overflow vs no problem!
...
(define (reverse-tr lst)
  (define (go acc a)
    (show-stack-info 'go (format "(~a, ~a)" acc a))
    (if (null? a)
        acc
        (go (cons (car a) acc) (cdr a))))
  (go '() lst))
