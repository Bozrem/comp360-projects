...
;;; Answer:
;;; Explanation (which x does (+ x 1) see in the let bindings?):
...
;;; What does this evaluate to?
;;; Answer:
;;; Explanation (how does let* differ from let here?):
...
;;; What does this evaluate to?
;;; Answer:
;;; Explanation (what value does b get? what value does the inner a have?):
...
;;; Answer:
;;; Explanation (which z does the lambda see — the global or local one?):
...
;;; (box1 'get)       =>
;;; ((box2 'set) 0)   => (what's the return? what's the effect?)
;;; (box2 'get)       =>
;;; (box1 'get)       =>
;;; Explanation (do box1 and box2 share their value variable?):
...
; (map 'todo '(1 2 3))        ; => '(101 102 103)

;;; 6b: Check if each number is positive
; (map 'todo '(-2 0 3 -1 5))  ; => '(#f #f #t #f #t)
...
; (filter 'todo '("hello" "" "world" "" "!"))   ; => '("hello" "world" "!")
...
; (map 'todo '(10 20 30 40))  ; => '(5 10 15 20)
...
; (define greeters (map 'todo '("Hello, " "Hi, " "Hey, ")))
